...
echo "HASH_x64=$(dotnet publish --no-restore --os linux --arch x64 /t:PublishContainer -p ContainerRegistry="$REGISTRY" -p ContainerRepository="$REPOSITORY" --getProperty:GeneratedContainerDigest src/AlbusKavaliro.Testcontainers.AzureApplicationInsights.Server/AlbusKavaliro.Testcontainers.AzureApplicationInsights.Server.csproj)" >> "$GITHUB_OUTPUT"
